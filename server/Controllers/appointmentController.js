...
export const getAvailableTokens = async (req, res) => {
  try {
    ...
    const bookedAppointments = await Appointment.countDocuments({ doctorId, appointmentDate: date });
...
export const createAppointment = async (req, res) => {
  try {
    ...
    const existingAppointment = await Appointment.findOne({ doctorId, userId, appointmentDate });

    if (existingAppointment) {
      return res.status(400).json({ message: "You have already booked an appointment for this doctor on this date." });
    }
    ...
    const bookedAppointments = await Appointment.countDocuments({ doctorId, appointmentDate });
...
export const cancelAppointment = async (req, res) => {
  try {
    ...
    if (doctor) {
      const remainingTokens = doctor.totalAppointments - bookedAppointments;
      res.status(200).json({ message: "Appointment canceled successfully", remainingTokens });
    } else {
      res.status(200).json({ message: "Appointment canceled successfully, but doctor not found" });
    }
  } catch (error) {
